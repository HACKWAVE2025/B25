...
async function getConsultationDetails(req, res, next) {
  try {
    ...
    const cons = await Consultation.findOne({ _id: id, hospitalId })
      .select('consultationId patientId chiefComplaint conditionType createdAt nurseId images status doctorId startedAt video')
      .lean();
    if (!cons) return res.status(404).json({ message: 'Consultation not found' });

    const patient = await Patient.findById(cons.patientId)
      .select('patientId personalInfo.firstName personalInfo.lastName personalInfo.age personalInfo.gender personalInfo.height personalInfo.weight personalInfo.phone photos conditionType')
      .lean();
  ...
  phone: patient.personalInfo.phone || null,   // <- added
